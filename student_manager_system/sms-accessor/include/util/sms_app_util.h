...
class SmsAppUtil {
  ...
  */
   ...
   */
  static std::vector<fs::path> GetFilesInDirectory(
      fs::path full_path, fs::path *ext_filter = nullptr);
  ...
  */
   ...
   */
  static int WriteUnicodeIni(std::wstring &path, boost::property_tree::wptree &pt);
   ...
   */
  static boost::property_tree::wptree ReadUnicodeIni(std::wstring &path);
