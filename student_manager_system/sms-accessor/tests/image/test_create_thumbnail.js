...
const GOYO19_APP_FOLDER = 'tests/test_data/data_test_create_thumbnail';
...
test.serial('createTumbnail JPG -> JPG', async t => {
  ...
  try {
    ...
    let height = 100;
    let quality = 75;
    const response = await TARGET.createThumbnail(sourceFile, destFile, width, height, quality);
...
test.serial('quality isn\'t string.', async t => {
  testGetConstructionsUtil.initialize(t);
  const TARGET = require(BOOKRACK_MODULE);
  TARGET.initialize(GOYO19_APP_FOLDER);
  TARGET
  ...
  try {
    ...
    t.is(e.message, "'args.quality' is not specified");
    t.is(e.type, "INVALID_COMMAND");
    t.pass('Success Not exist file');
  } finally {
    await TARGET.finalize();
...
test.serial('TPI file.', async t => {
  ...
  try {
    ...
    let height = 100;
    let quality = 75;
    await TARGET.createThumbnail(sourceFile, destFile, width, height, quality);
    t.fail('Not throw exception');
  } catch (e) {
